...
contract Locker is Ownable {
  ...
  //  event Process(uint256 _balance, address _feeWallet);

event NativeCurrencyLocked(address _owner, uint256 _targetTimestamp);
  ...
  event LockTimeExtended(address _owner, uint256 _newTimeStamp);

event Claimed(address _owner, uint256 _vaultId);
  ...
  //   }
    ...
    function lock(
        ...
        returns (bool) {
          emit TokensLock(msg.sender, tokenCount , targetTimestamp, tokenContract);
            return vaultIndex;
        } catch (bytes memory) {
    ...
    function lockNativeCurrency(uint256 targetTimestamp) public payable {
        ...
        checkpoint.releaseTargetTimestamp = targetTimestamp;
        checkpoint.claimed = false;
        emit NativeCurrencyLocked(msg.sender, targetTimestamp);
    }

    function extendLockTime(uint256 _vaultId, uint256 _targetTime) external   {
        VestedTokenVault storage vault = userLocks[msg.sender].userVaults[_vaultId];
        ReleaseCheckpoint storage checkpoint = vault.checkpoints[0];
        require(_targetTime > checkpoint.releaseTargetTimestamp, "New unlock time must be greater than previous");
         require(block.timestamp < _targetTime, "New unlock time must be greater than the current time");
        checkpoint.releaseTargetTimestamp = _targetTime;
        emit LockTimeExtended(msg.sender, _targetTime);
    }
    ...
    function claimByVaultId(uint256 vaultId) public payable returns (bool) {
        ...
        if (vault.nativeToken) {
            payable(msg.sender).transfer(vault.checkpoints[0].tokensCount);
            ...
            returns (bool) {
                checkpoint.claimed = true;
                emit Claimed(msg.sender, vaultId );
                return true;
            } catch (bytes memory) {
